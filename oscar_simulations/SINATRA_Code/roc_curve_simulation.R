  ...
  if (type == 'vertex'){
    ...
    while (i<runs+1){
      roc_curve = try(generate_ROC_with_coned_directions(nsim = nsim, curve_length = curve_length, grid_size = grid_size, distance_to_causal_point = distance_to_causal_point,
                                                         causal_points = causal_points,shared_points = shared_points,num_directions = num_directions,
                                                         eta = eta,truncated = truncated, two_curves = two_curves, 
                                                         ball = ball, ball_radius = ball_radius,type = type, min_points = min_points,
                                                         cap_radius = cap_radius,directions_per_cone = directions_per_cone))
  ...
  if (type == 'cone'){
    ...
    while (i<runs+1){
      roc_curve = try(generate_ROC_with_coned_directions(nsim = nsim, curve_length = curve_length, grid_size = grid_size, distance_to_causal_point = distance_to_causal_point,
                                                         ...
                                                         eta = eta,truncated = truncated, two_curves = two_curves, 
                                                         ball = ball, ball_radius = ball_radius, type = type, min_points = min_points,
                                                         cap_radius = cap_radius,directions_per_cone = directions_per_cone, radius = radius))
      if (inherits(roc_curve,'try-error')){
        next
      }
      else{
...
generate_ROC_with_coned_directions <- function(nsim = 10, curve_length = 25, grid_size = 25, distance_to_causal_point = 0.1, 
  ...
  data <- create_data_normal_fixed(num_sim = nsim, dir = directions, curve_length = curve_length,shared_points = shared_points,
                                   causal_points = causal_points,grid_size = grid_size,eta = eta,ball_radius = ball_radius,ec_type = ec_type)
  ...
  if (type == 'vertex'){
    if (two_curves == TRUE){
      roc_curve1 =  compute_roc_curve_vertex(data_points = complex_points1, class_1_causal_points = data$causal_points1, class_2_causal_points = data$causal_points2,
                                             curve_length = curve_length, distance_to_causal_point = distance_to_causal_point, rate_values = rate_values, grid_size = grid_size,
                                             eta = eta, directions_per_cone = directions_per_cone, directions = directions, class = 1,truncated = 0, 
                                             ball_radius = ball_radius)
      roc_curve1 = cbind(roc_curve1, rep(1,dim(roc_curve1)[1]))
      roc_curve1 = cbind(roc_curve1,(1:dim(roc_curve1)[1]))
      
      roc_curve2 =  compute_roc_curve_vertex(data_points = complex_points2, class_1_causal_points = data$causal_points1, class_2_causal_points = data$causal_points2,
                                             curve_length = curve_length, distance_to_causal_point = distance_to_causal_point, rate_values = rate_values, grid_size = grid_size,
                                             eta = eta, directions_per_cone = directions_per_cone, directions = directions,class = 2,truncated = 0,
                                             ball_radius = ball_radius)
      ...
      roc_curve = compute_roc_curve_vertex(data_points = data$complex_points, class_1_causal_points = data$causal_points1, class_2_causal_points = data$causal_points2,
                                           curve_length = curve_length, distance_to_causal_point = distance_to_causal_point, rate_values = rate_values, grid_size = grid_size,
  ...
  if (type == 'feature'){
    roc_curve = compute_roc_curve_features(data_points = data$complex_points, class_1_causal_points = data$causal_points1, class_2_causal_points = data$causal_points2,
                                           curve_length = curve_length, distance_to_causal_point = distance_to_causal_point, rate_values = rate_values, grid_size = grid_size,
                                           eta = eta, directions_per_cone = directions_per_cone, class = 0, truncated = truncated,
                                           ball = ball,ball_radius = ball_radius, 
                                           dir = directions, min_points = min_points)
    roc_curve = cbind(roc_curve,(1:dim(roc_curve)[1]))
    return(roc_curve)
  }
  if (type == 'cone'){
    print('cone')
    roc_curve = compute_roc_curve_cones(data_points = data$complex_points, class_1_causal_points = data$causal_points1, class_2_causal_points = data$causal_points2,
                                        curve_length = curve_length, distance_to_causal_point = distance_to_causal_point, rate_values = rate_values, grid_size = grid_size,
                                        eta = eta, directions_per_cone = directions_per_cone, class = 0, truncated = truncated,
                                        ball = ball, ball_radius = ball_radius,
...
compute_roc_curve_vertex = function(data_points,class_1_causal_points,class_2_causal_points,distance_to_causal_point = 0.1,
  ...
  for (j in 1:length(data_points)){
    ...
    complex=MatrixtoSimplicialComplexTriangular(predictions,grid_length=grid_size)
    ...
    for (i in 1:num_vertices){
      ...
      dist1=apply(X = class_1_causal_points[,1:2],MARGIN = 1,FUN = difference,y=complex$Vertices[i,1:2])
      dist2=apply(X = class_2_causal_points[,1:2],MARGIN = 1,FUN = difference,y=complex$Vertices[i,1:2])
    ...
    if (truncated == 0){
      for (threshold in quantile(rate_values,probs = seq(1,0,length.out = length(rate_values))) ){
        ...
        #sink("/dev/null")
        rate_positive_vertices<- compute_selected_vertices_cones(dir = directions, complex = complex, rate_vals = rate_values, len = curve_length, threshold = threshold,
...
compute_roc_curve_features <- function(data_points,class_1_causal_points,class_2_causal_points,distance_to_causal_point = 0.1,
...
compute_roc_curve_cones <- function(data_points,class_1_causal_points,class_2_causal_points,distance_to_causal_point = 0.1,
                                    rate_values,grid_size,eta = 0.1,directions_per_cone, curve_length, truncated = 0,
  ...
  for (j in seq(1,length(data_points),2)){
    ...
    #Count the number of total reconstructions
