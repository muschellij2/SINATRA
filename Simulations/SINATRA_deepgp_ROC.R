...
### Do the inference
# export the data to sghmc-dgp
write.matrix(ec_curve_data, file = "sinatra_ECT_dataROC.txt", sep = ",")



# import the posterior results and the data matrix
posterior_samples <- read.table('Results/posteriorsamples_sinatraROC.txt', header = FALSE)
posterior_samples <- as.matrix(posterior_samples)

X <-  read.table('Results/designmatrix_sinatraROC.txt', header = FALSE)
X <- as.matrix(X)
