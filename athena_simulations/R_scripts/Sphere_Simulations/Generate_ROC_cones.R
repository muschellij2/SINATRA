...
simulation_results <- foreach(i=1:n.simulations, .combine = 'rbind', .noexport = c('GaussKernel')) %:% 
  foreach(j=c(10,30,50,70,90), .combine = 'rbind', .noexport = c('GaussKernel')) %dopar% {
...
ROC_curve_plt <- ggplot(data <- rdfmeans[rdfmeans$Class == 1,],aes(x = FPR, y = TPR, color = Num_Cones)) +
  ...
  labs(x = "FPR", y = "TPR") +
  ggtitle(sprintf("hard case many cones")) +
...
sim_results_file = sprintf("~/data/SINATRA/new/data_ROC_large_cones_causal%d_shared%d_region_size%s.RData",
                           num_causal_region,num_shared_region,causal_points)
save(simulation_results, file = sim_results_file)

# save the dataframe
df_results_file = sprintf("~/data/SINATRA/new/df_ROC_large_cones_causal%d_shared%d_%s.RData",
