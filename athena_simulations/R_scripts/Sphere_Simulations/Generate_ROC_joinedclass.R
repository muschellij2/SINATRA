...
simulation_results <- foreach(i=1:n.simulations, .combine = 'rbind', .noexport = c('GaussKernel')) %:% 
  foreach(j=c(1,5,20,50), .combine = 'rbind', .noexport = c('GaussKernel')) %dopar% {
    ...
    res <- tryCatch( generate_ROC_with_coned_directions(nsim = 25, curve_length = 30, grid_size = 25, distance_to_causal_point = 0.1, 
                                                        ...
                                                        truncated = 250, two_curves = FALSE, ball = TRUE, ball_radius = 1.5, type = 'vertex',
    ...
    rdf <- plyr::rename(rdf,c("X1" = "FPR","X2" = "TPR","X3" = "Index","X4" = "Num_Cones","X5" = "Trial"))
  }
...
### Plot results ###
ROC_curve_plt <- ggplot(data <- rdfmeans,aes(x = FPR, y = TPR, color = Num_Cones)) +
  geom_line(stat = "identity") +
  labs(x = "FPR", y = "TPR") +
  ggtitle(sprintf("causal_regions:3,shared_regions:6_5")) +
