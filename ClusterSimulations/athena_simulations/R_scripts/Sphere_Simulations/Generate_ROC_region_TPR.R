...
simulation_results <- foreach(i=1:n.simulations, .combine = 'rbind', .noexport = c('GaussKernel')) %:% 
  foreach(j=c(1,10,20), .combine = 'rbind', .noexport = c('GaussKernel')) %dopar% {
    
    set.seed(8*i+j)
    
    res <- tryCatch( generate_ROC_with_coned_directions(nsim = 5, curve_length = 30, grid_size = 25, distance_to_causal_point = 0.1, 
                                                        causal_points = causal_points,shared_points = shared_points, desired_num_cones = j, eta = 0.1, 
                                                        truncated = 250, two_curves = TRUE, ball = TRUE, ball_radius = 1.5, type = 'cusp',
...
sim_results_file = sprintf("~/data/SINATRA/new/data_region_ROC_causal%d_shared%d_region_size%s.RData",
                           num_causal_region,num_shared_region,causal_points)
save(simulation_results, file = sim_results_file)

# save the dataframe
df_results_file = sprintf("~/data/SINATRA/new/df_region_ROC_causal%d_shared%d_%s.RData",
