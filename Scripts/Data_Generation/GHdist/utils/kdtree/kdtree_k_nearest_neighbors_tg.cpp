...
void retrieve_point( const mxArray* matptr, int& pointIdx, int& npoints, vector<double>& point ){
    // check that I actually received something
    if( matptr == NULL )
        mexErrMsgTxt("vararg{2} is missing\n");

    if( (mxGetN(matptr)!=point.size()) && (mxGetM(matptr)!=point.size()) )
    	mexErrMsgTxt("vararg{2} must be a [kxN] or a [Nxk] point\n");    	
    
    // retrieve point
    double* data = mxGetPr(matptr);
    
    for(int dim=0; dim < point.size(); dim++)
		point[dim] = data[pointIdx+npoints*dim];
}
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
    ...
    plhs[0] = mxCreateDoubleMatrix(npoints, k, mxREAL);
    plhs[1] = mxCreateDoubleMatrix(npoints, k, mxREAL);  
    double* indexes = mxGetPr(plhs[0]);
    double* dists   = mxGetPr(plhs[1]);
    
    for (int i=0; i<npoints; i++) {
        // retrieve the i-th point
        vector< double > query(ndims,0);
        retrieve_point(prhs[1], i, npoints, query);
        ...
        for (int j=0; j<idxsInRange.size(); j++){
            indexes[i+j*npoints] = idxsInRange[j] + 1;
            dists[i+j*npoints] = distances[j];
        }   
    }
}
#endif
