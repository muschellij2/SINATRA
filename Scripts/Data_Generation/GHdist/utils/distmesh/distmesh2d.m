...
while 1
  ...
  if max(sqrt(sum((p-pold).^2,2))/h0)>ttol           % Any large movement?
    ...
    t=t(feval(fd,pmid)<-geps,:);         % Keep interior triangles
...
%     drawnow
  ...
  d=feval(fd,p); ix=d>0;                 % Find points outside (d>0)
%   d=feval(fd,p,varargin{:}); ix=d>0;                 % Find points outside (d>0)
  dgradx=(feval(fd,[p(ix,1)+deps,p(ix,2)])-d(ix))/deps; % Numerical
  dgrady=(feval(fd,[p(ix,1),p(ix,2)+deps])-d(ix))/deps; %    gradient
