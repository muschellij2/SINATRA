...
for a = 1:100
    ...
    data_path = ['/Users/Bruce/Documents/new_aligned_shapesv3/V13_3peak_2gp_v' num2str(a) '_bw/mesh/'];
...
%% Plot the second highlighted function
figure;
para.view_angle = [0, 270]; 
fa = (shapes{1}.evecs(:, 1:ndim)*U.cbases{1}(:, 1:nCLB)*u(:, 2)).^2;
fb = (shapes{NumPerGroup+1}.evecs(:, 1:ndim)*U.cbases{NumPerGroup+1}(:, 1:nCLB)*u(:, 2)).^2; 
h(1) = subplot(121); plot_function(shapes{1}, fa, para); title('Area Diff Second: Class A'); 
h(2) = subplot(122); plot_function(shapes{NumPerGroup+1}, fb, para); title('Area Diff Second: Class B');
cameratoolbar
Link = linkprop(h, {'CameraUpVector', 'CameraPosition', 'CameraTarget', 'CameraViewAngle'});
setappdata(gcf, 'StoreTheLink', Link);
