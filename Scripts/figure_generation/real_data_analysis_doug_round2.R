...
fruit1_vert = compute_selected_vertices_cones(dir = dirs, complex =fruit_1, rate_vals = fruit_veg_new$Rate2[,2], len = len, threshold = ((directions_per_cone)/(dim(fruit_veg_new$Rate2)[1])),
                                              cone_size = directions_per_cone,ball_radius = ball_radius, ball = ball, radius = 1)
...
fruit_heat =  reconstruct_vertices_on_shape(dir = dirs,complex = fruit_1,rate_vals = fruit_veg_new$Rate2[,2],
                                            len = len,cuts = 100,cone_size = directions_per_cone,ball_radius = ball_radius, ball = ball,radius =1)
...
mfrow3d(1,2)
plot3d(fruit1,col = fruit_colors_heat, back="lines", specular="black", axes = FALSE)
#plot_selected_landmarks(fruit1,fruit1_vert,num_landmarks = 20)
rgl.viewpoint(userMatrix = rotation_matrix)



plot3d(veg1,col = veg_colors_heat, back="lines", specular="black", axes = FALSE)
...
fruit_colors1.5=colfunc(max(fruit_heat1.5[,1]) - min(fruit_heat1.5[,1]))[fruit_heat1.5[,1]- min(fruit_heat1.5[,1])]
bug_colors1.5=colfunc(max(bug_heat1.5[,1])- min(bug_heat1.5[,1]))[bug_heat1.5[,1]- min(bug_heat1.5[,1])]
...
veg_colors2=colfunc(max(veg_heat2[,1]) - min(veg_heat2[,1]))[veg_heat2[,1]- min(veg_heat2[,1])]
bug_colors2=colfunc(max(bug_heat2[,1])- min(bug[,1]))[bug_heat2[,1]-min(bug_heat2[,1])]
